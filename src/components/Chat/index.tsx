...
export default function ChatReclutadores() {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            const errorMessage = { role: 'assistant', text: "Hubo un error al procesar tu pregunta." };
    ...
    const handleKeyPressPersonalData = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
        ...
        <>
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid gap-2">
                            <Input
                                ...
                                onKeyPress={handleKeyPressPersonalData}
                                className={showErrors && (!userData.email || (userData.email && !isValidEmail(userData.email))) ? 'border-red-500' : ''}
                            />
                            {showErrors && !userData.email && (
                                <span className="text-red-500 text-sm">El correo electrónico es obligatorio</span>
                            )}
                            {showErrors && userData.email && !isValidEmail(userData.email) && (
                                <span className="text-red-500 text-sm">Por favor, ingresa un correo electrónico válido</span>
                            )}
                        </div>
                    </div>
            ...
            <div className="flex flex-col h-screen p-4">
                <div className="flex-grow overflow-auto border p-4 rounded-xl">
                    {messages.map((msg, i) => (
                        <div key={i} className={`flex items-center my-6 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className={`relative max-w-[70%] ${msg.role === 'user' ? 'mr-3' : 'ml-3'}`}>
